...
pm1=now - relativedelta(months=2) # converts to previous month
pm2=now - relativedelta(months=3) # converts to previous month
timeplist=[str(pm2.year)+str(pm2.month),str(pm1.year)+str(pm1.month),str(pm.year)+str(pm.month)]

timestr= pm.strftime("%Y") + ' ' + pm2.strftime("%b") +  '-'  + pm.strftime("%b")
timestrt= pm.strftime("%Y") + '_' + pm2.strftime("%b") +  '-' + pm.strftime("%b")
print('time: '+timestr)
...
#os.system(cmd)
# --> copying should not be necessary since it is already done for the monthly report, therfore quarterly should be executed after monthly

# plotpath
#ppath = '/lustre/storeA/project/fou/hi/waveverification/Arc-MFC/'+timestrt+'/'
ppath = '/lustre/storeA/project/fou/om/waveverification/Arc-MFC/quarterly/'+timestrt+'/'
